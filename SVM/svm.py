...
def smo_simple(data_mat_in,label_mat_in,c,toler,max_iter):
    ...
    while(iter<max_iter):
        ...
        for i in range(m):
            yi=multiply(alpha,label_mat).T*(data_mat*data_mat[i,:].T)+b
            Ei=yi-label_mat[i]
            if ((label_mat[i]*Ei<=-toler) and (alpha[i]<c)) or ((label_mat[i]*Ei>=-toler) and (alpha[i]>0)) or ((label_mat[i]*Ei==-toler) and ((alpha[i]==0) or (alpha[i]==c))):
                j=select_j_rand(i,m)
                yj=multiply(alpha,label_mat).T*(data_mat*data_mat[j,:].T)+b
                Ej=yj-label_mat[j]
                ...
                if abs(alpha[j]-alphaj_old)<0.00001:
                    ...
                    continue
                alpha[i]+=label_mat[i]*label_mat[j]*(alphaj_old-alpha[j])
                b1=b-Ei-label_mat[i]*(alpha[i]-alphai_old)*(data_mat[i,:]*data_mat[i,:].T)-label_mat[j]*(alpha[j]-alphaj_old)*(data_mat[i,:]*data_mat[j,:].T)
                b2=b-Ej-label_mat[i]*(alpha[i]-alphai_old)*(data_mat[i,:]*data_mat[j,:].T)-label_mat[j]*(alpha[j]-alphaj_old)*(data_mat[j,:]*data_mat[j,:].T)
